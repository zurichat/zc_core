...
func GetandSetDB(collection string, expiry int) {
	grt, ccfilter := make(map[string]interface{}), make(map[string]interface{})
...
func AuthorizeOrigin(r *http.Request) error {
	...
	if !status {
		ncCount, state := CheckOriginConnections(origin)
		if !state {
			return ConnectLimitError(ncCount)
		}
...
func UserIDFromSession(sessionData *auth.ResToken, conf *utils.Configurations) (string, error) {
	...
	ee := mapstructure.Decode(sessionData, &data)
	if ee != nil {
		return "", ee
	}
